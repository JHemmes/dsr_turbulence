...
class Controller(object):
    ...
    def __init__(self, sess, prior, enforce_sum, seed, debug=0, summary=True,
        ...
        with tf.name_scope("controller"):
            ...
            if enforce_sum:
                cell = LinearWrapper(cell=cell, output_size=n_choices*4)
            else:
            ...
            if enforce_sum:  # stack four priors:
                initial_prior = tf.concat([initial_prior for _ in range(4)], -1)
            ...
            if enforce_sum:
                ...
                def loop_fn(time, cell_output, cell_state, loop_state):
                    if cell_output is None:  # time == 0
                        ...
                        obs_tas = (tf.TensorArray(dtype=tf.int32, size=0, dynamic_size=True, clear_after_read=True),
                                   # Action inputs
                                   tf.TensorArray(dtype=tf.int32, size=0, dynamic_size=True, clear_after_read=True),
                                   # Parent inputs
                                   tf.TensorArray(dtype=tf.int32, size=0, dynamic_size=True,
                                                  clear_after_read=True))  # Sibling inputs
                        priors_ta = tf.TensorArray(dtype=tf.float32, size=0, dynamic_size=True, clear_after_read=True)
                        prior = initial_prior
                        lengths = tf.ones(shape=[self.batch_size], dtype=tf.int32)
            ...
            # set up actions, obs and priors to be returned
            if enforce_sum:
        ...
        if enforce_sum:
                ...
                with tf.name_scope(name):
                    batch_ph = {
                        ...
                        "obs" : ((tf.placeholder(tf.int32, [None, max_length]),
                                 ...
                                 tf.placeholder(tf.int32, [None, max_length]),
                                 tf.placeholder(tf.int32, [None, max_length]))),
                        "priors" : tf.placeholder(tf.float32, [None, max_length, n_choices*4]),
            ...
            def make_neglogp_and_entropy(B, return_tuple=False):
